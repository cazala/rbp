...
Inspect the publicly verified source on [Etherscan](https://etherscan.io/address/0xb69aF08877a0C417169135D6710Bca4840CCCdE1#code) or [Sourcify](https://repo.sourcify.dev/1/0xb69aF08877a0C417169135D6710Bca4840CCCdE1). Etherscan reports `ResurrectOnchainSite`, Solidity `v0.8.24+commit.e11b9ed9`, optimization enabled with 20,000 runs, no proxy, and the complete 2,432-byte constructor arguments. Sourcify independently reports creation and runtime matches under match ID `48092080`. The complete authoritative record—including every chunk address, transaction, block, payload length, and file hash—is [`deployments/ethereum-mainnet-explorer.json`](../deployments/ethereum-mainnet-explorer.json). It is published as `@resurrect-protocol/contracts/deployments/ethereum-mainnet-explorer.json`.
...
Only the router is a compiled Solidity contract with a conventional source-verification target. The 25 data addresses are intentionally raw runtime bytecode generated by `ResurrectBytecodeStorage.write`; submitting a Solidity contract as their “source” would be inaccurate. Their verification evidence is the source-verified router's immutable pointers plus the recorded code addresses, payload lengths, hashes, and byte-for-byte reconstruction checks below.
...
- Etherscan and Sourcify independently accepted the pinned source and compiler settings for the router;
